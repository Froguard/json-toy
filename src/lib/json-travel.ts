...
export interface TravelCallbackInfo {
  keyPath: string;
  depth: number;
  type: string;
  isCircular: boolean;
...
export type TravelCallback = (key: string, value: any, info?: Partial<TravelCallbackInfo>) => void;
 ...
 */
export function travelJson(json: any, cb?: TravelCallback, rootAlias?: string, safeMode: boolean = true): string[] {
  if (!isObject(json)) {
