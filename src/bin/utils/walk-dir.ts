 ...
 * @returns {JSON|String}
 */
function dir2Json(rootPath: string, options?: Dir2JsonOptions): any {
  // eslint-disable-line
  // root
  ...
  const outExcludeDir = exclude.outExcludeDir;
    ...
    if (!maxDepth || (maxDepth && curDepth <= maxDepth)) {
      ...
      if (isArray(dirs)) {
        if (dirs.length >= 30) {
          console.log(
            `${colorful.warn(
              'Too many files|directories was found, type-in an exact target to simplify or reduce result.',
