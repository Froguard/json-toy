...
// 类型名称集合（已精简）
// type TypeName = 'arguments' | 'array' | 'date' | 'error' | 'object' | 'function' | ...
...
['arguments', 'array', 'date', 'error', 'syntaxError', 'typeError', 'rangeError', 'regExp', 'symbol', 'set', 'weakSet', 'map', 'weakMap'].forEach(t => {
...
function _isJSON(value: any, visited: any[] = []): boolean {
  ...
  return (
    ...
    ((isObject as any).isFlat(value) &&
      Object.keys(value).every(key => {
        const $ = Object.getOwnPropertyDescriptor(value, key);
        return $ && (!isObject($.value) || !visited.includes($.value)) && !('get' in $) && !('set' in $) && _isJSON($.value, visited);
      }))
  );
}
