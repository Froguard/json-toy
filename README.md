...
- 1.**treeify(jsonObj1)** &nbsp; or &nbsp;&nbsp; `$ jts ./path/1.json` (cmd line)&nbsp;&nbsp; convert a jsonObj to a tree-like string to print out
...
- 6.**travelJson(jsonObj,cb)** &nbsp; **Safe** recursive walk of a jsonObj,even though obj is circular!

- 7.**getValByKeyPath(jsonObj,'x.y.z.1.2')** &lt;=&gt; **jsonObj.x.y.z\[1]\[2]**

- 8.**checkCircular(obj)** check the obj,and return some useful info
...
1. use `treeify(jsonObj,options)` to convert json to tree-string
...
`treeify(jsonObj)`，support multiple primitive type
...
- **copy-paste-feature**,json-toy used system shell command line to exec copy,so you may get some string pre space char '' was removed by sys.
  ...
  eh...You'd better paste in a text file like _.md or _.txt or others ext
