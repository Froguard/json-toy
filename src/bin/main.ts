...
const argJ = args.j || args.json || (Type.isArray(args._) ? args._[0] : args._);
let argD = args.d || args.dir;
let argM = args.m || args.max;
argM = Type.isBoolean(argM) ? 5 : parseInt(String(argM)) || 5;
argM = argM < 0 ? 0 : argM;
const needCopy = args.c === undefined && args.copy === undefined ? true : !!(args.c || args.copy);
  ...
  if (jsonObj !== false) {
    ...
    if (treeStr) {
      ...
      if (!(Type.isObject as any).isEmptyOwn(jsonObj) && needCopy) {
        ...
        copyTxtToClipboard(treeStr).then(() => console.log(colorful.success('Copy tree-string success!')));
      }
    }
  }
}
